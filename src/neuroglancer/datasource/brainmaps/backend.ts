...
import {decodeMorton3, kInfinityVec, kZeroVec, vec3, vec3Key, zorderLessThan} from 'neuroglancer/util/geom';
...
function decodeMultiscaleManifestChunk(chunk: BrainmapsMultiscaleManifestChunk, response: any) {
  ...
  const fragmentIdAndCorners =
      Array.from(fragmentSupervoxelIds.entries())
          .map(
              ([id, supervoxelIds]) =>
                  ({fragmentId: id, corner: getFragmentCorner(id), supervoxelIds}));
  fragmentIdAndCorners.sort((a, b) => {
    return zorderLessThan(
