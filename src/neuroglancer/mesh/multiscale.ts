...
import {isAABBVisible, mat4, vec3, zorderLessThan} from 'neuroglancer/util/geom';
...
export function getDesiredMultiscaleMeshChunks(
  ...
  function handleChunk(lod: number, chunkIndex: number, chunkEndIndexBound: number) {
    const lodMask = ~0 << lod >>> 0;
    const size = 1 << lod;
    ...
    if (isAABBVisible(xLower, yLower, zLower, xUpper, yUpper, zUpper, clippingPlanes)) {
      ...
      if (lod > 0 && (lodScale == 0 || pixelSize * detailCutoff < lodScale)) {
        do {
          chunkIndex = handleChunk(lod - 1, chunkIndex, chunkEndIndex);
  ...
  handleChunk(maxLod, 0, numChunks);
}
